...
(function (global, factory) {
  ...
  var isUsingMicroTask = false;  //标记是否使用的是微任务
  // 存放异步执行的回调
  var callbacks = []; 
  ...
  *作为“深层”依赖关系收集
  ...
  function resolveConstructorOptions (Ctor) {
    var options = Ctor.options;
    if (Ctor.super) {//如果有超类，
      var superOptions = resolveConstructorOptions(Ctor.super);//获取超类的组件选项
      var cachedSuperOptions = Ctor.superOptions;//拿到构造函数上原先存的超类组件选项
  ...
  function initExtend (Vue) {
    ...
    Vue.extend = function (extendOptions) {
      ...
      var Super = this;//超类，一般情况下就是Vue，除非多次继承，这种情况几乎不会出现
      var SuperId = Super.cid;//超类的cid
  ...
  //keep-alive组件销毁时，清除缓存
    ...
    render: function render () {
      var slot = this.$slots.default;  //default插槽，值是一个数组，成员是 vnode
      var vnode = getFirstComponentChild(slot);   //拿到插槽的第一个是组件的 vnode
  ...
  // 判断input的type是否相同或者都是'text,number,password,search,email,tel,url'类型中的一个输入框
  ...
  //创建打补丁函数，返回一个函数，用于打补丁  函数名 patch
  ...
  // 遍历children节点，检测是否有重复的key属性值，并给出警告
  ...
  // 打补丁函数
  // 参数依次为 旧的vnode| dom节点, 新的vnode, web端 :hydrating=false , removeOnly=false
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
      // 如果传入新的vnode是null或者undefined，销毁组件
      if (isUndef(vnode)) {
  ...
  // 子组件初始化会走这里，创建元素
      if (isUndef(oldVnode)) {
        ...
        // 如果oldVnode有nodeType属性，说明传入的oldVnode是真实DOM
        var isRealElement = isDef(oldVnode.nodeType);
        // 如果第一个参数不是真实DOM并且新旧vnode值得比较，就调用 patchVnode 方法打补丁
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          // patch existing root node
          patchVnode(oldVnode, vnode, insertedVnodeQueue, null, null, removeOnly);
        } else {//要么是真实DOM，要么新旧vnode不值得比较走这里
          if (isRealElement) {  //第一个参数是真实DOM走这里
            // mounting to a real element
            // check if this is server-rendered content and if we can perform
            // a successful hydration.
            // 挂载真实DOM，检查如果是服务端渲染内容 ，并且我们可以做一个成功的混合
            // SSR_ATTR属性是服务端渲染标记，如果是服务端渲染设置hydrating参数为true
          ...
          var parentElm = nodeOps.parentNode(oldElm);  //这句是找到旧DOM元素的父元素
  ...
  var createCompiler = createCompilerCreator(function baseCompile (
    ...
    var ast = parse(template.trim(), options);//将模板解析成抽象语法树
